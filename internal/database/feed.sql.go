...
SELECT id, created_at, updated_at, name, url, user_id, last_fetched_at FROM feed ORDER BY last_fetched_at ASC NULLS FIRST LIMIT $1
`
...
UPDATE feed SET last_fetched_at = NOW(), updated_at= NOW() WHERE id=$1 RETURNING id, created_at, updated_at, name, url, user_id, last_fetched_at
`
